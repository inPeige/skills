...
5. **数字就是数字**（`5000` 就写 `5000`，别转成「五千」）——多一次转写就多一次写错的机会；
   TTS 能正确读阿拉伯数字，逐字锚定也不要求汉字
...
把 W 也当失败——因为「零标点」这类**事实红线是警告级别的**，
不加 `--strict` 会带着 `（207,102）` 一路放行到配音那步才炸。
交付标准是 **`--strict` 也全过**。
